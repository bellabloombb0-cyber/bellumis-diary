...
const links = [["Über mich", "philosophy"], ["Unterwegs", "journey"], ["Entdecken", "explore"], ["Musik", "music"], ["Portugal", "portugal"], ["Kontakt", "contact"]];
export function Navbar() {
  ...
  return <header className="fixed inset-x-0 top-0 z-50 border-b border-ink/8 bg-mist/88 backdrop-blur-md"><nav className="mx-auto flex h-18 max-w-7xl items-center justify-between px-5 lg:px-8">
    <a href="#top" aria-label="Bellumi’s Diary – Startseite" className="flex items-center gap-2 text-ink"><PassifloraLogo className="h-8 w-8" /><span className="font-[family-name:var(--font-display)] text-2xl tracking-tight">Bellumi’s Diary</span></a>
    ...
    <button className="rounded-full border border-ink/15 px-3 py-2 text-xs font-bold md:hidden" onClick={() => setOpen(!open)} aria-expanded={open}>Menü</button>
